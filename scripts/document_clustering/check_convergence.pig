prior_centers  = LOAD '$PRIOR_CENTERS'  AS (doc_id:chararray, norm_sq:double, vector:bag {t:tuple (token:chararray, weight:double)});
latest_centers = LOAD '$LATEST_CENTERS' AS (doc_id:chararray, norm_sq:double, vector:bag {t:tuple (token:chararray, weight:double)});
