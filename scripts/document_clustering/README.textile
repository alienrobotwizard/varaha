...
<pre><code>
$VARAHA_HOME/bin/varaha -p DOCS=/path/to/my_docs -p NDOCS=<num_docs> -p TFIDF=/path/to/output tfidf.pig
</code></pre>
...
<pre><code>
$VARAHA_HOME/bin/varaha -p TFIDF=/path/to/tfidf-output -p CENTERS=/path/to/output sample_k_centers.pig
</code></pre>
...
<pre><code>
$VARAHA_HOME/bin/varaha -p TFIDF=/path/to/tfidf-output -p CURR_CENTERS=/path/to/current-centers -p NEXT_CENTERS=/path/to/next-centers cluster_documents.pig
</code></pre>
