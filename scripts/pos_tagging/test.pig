...
reviews = LOAD 'test.avro' USING AvroStorage();
reviews = LIMIT reviews 1000;
foo = FOREACH reviews GENERATE business_id, varaha.text.StanfordTokenize(text) AS tagged;
DUMP foo

reviews = LOAD 'test.avro' USING AvroStorage();
reviews = LIMIT reviews 1000;bar = FOREACH reviews GENERATE business_id, FLATTEN(varaha.text.SentenceTokenize(text)) AS tokenized_sentences;
